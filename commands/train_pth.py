...
@click.command()
@click.option("--train-dir", help="Path to the labelmg img+xml folder for training",
              required=True)
@click.option("--val-dir", help="Path to the labelmg img+xml folder for validation",
              required=True)
@click.option("--save-dir", help="model save dir",
              required=True)
def train_frcnn(train_dir, val_dir, save_dir):
    ...
    model_path = f"{save_dir}/faster-rcnn-{timestamp}.pth"
